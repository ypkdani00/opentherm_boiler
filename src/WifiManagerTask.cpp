...
// see https://github.com/s00500/ESPUI/blob/master/examples/completeExample/completeExample.cpp
...
UnsignedIntParameter *wmIndoorSensorPin[8];
...
void WifiManagerTask::setup()
{
  ...
  Log.sinfoln("WIFI", PSTR("Config debug port"));
  ...
  {
    wmIndoorSensorPin[i] = new UnsignedIntParameter("indoor_sensor_pin", "Indoor sensor GPIO", settings.sensors.indoor[i].pin, 2);
    wm.addParameter(wmIndoorSensorPin[i]);
  }
...
void WifiManagerTask::saveParamsCallback()
{
  ...
  for (int i = 0; i < INDOOR_SENSOR_NUMBER; i++)
  {
    if (wmIndoorSensorPin[i]->getValue() != settings.sensors.indoor[i].pin)
    {
      changed = true;
      needRestart = true;
      settings.sensors.indoor[i].pin = wmIndoorSensorPin[i]->getValue();
    }
  }
  ...
  for (int i = 0; i < INDOOR_SENSOR_NUMBER; i++)
  {
    Log.sinfo(
        "WIFI",
        PSTR("  Indoor %d sensor pin: %d\r\n"),
        settings.sensors.indoor[i].pin);
  }
